...
| 偏差项 | 宪法条款                  | 偏差内容                                                                                                              | 理由                                                                                                                                                                                      | 影响                                                                                     |
| ------ | ------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| D-001  | 开发工作流 — 图片资产命名 | 图片文件名保留设计师原始命名（PascalCase / 混合大小写，如 `Aura_index_banner01.webp`），未转换为宪法要求的 kebab-case | 设计稿素材已按此命名规范交付，`src/config/home.ts` 数据配置中统一引用原始文件名；批量重命名会导致设计师协作摩擦且无功能收益；文件名不影响 SEO（图片通过 `next/image` 输出带 hash 的 URL） | 低 — 仅影响 `public/images/01Home/` 目录下的静态资产文件名，不影响运行时行为、性能或 SEO |
